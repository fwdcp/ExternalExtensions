...
public:
	...
	virtual bool					HandleShotImpactingWater( const FireBulletsInfo_t &info,
		const Vector &vecEnd, ITraceFilter *pTraceFilter, Vector *pVecTracerDest ) = 0;
...
public:
	...
	virtual void					ChangeTeam( int iTeamNum ) = 0;			// Assign this entity to a team.
	virtual int						GetRenderTeamNumber( void ) = 0;
	virtual bool					InSameTeam( C_BaseEntity *pEntity ) = 0;	// Returns true if the specified entity is on the same team as this one
...
public:
	...
	virtual void					AddColoredDecal( const Vector& rayStart, const Vector& rayEnd,
		const Vector& decalCenter, int hitbox, int decalIndex, bool doTrace, trace_t& tr, Color cColor, int maxLODToDecal = ADDDECAL_TO_ALL_LODS ) = 0;
...
public:
	...
	virtual const QAngle&	EyeAngles( void ) = 0;		// Direction of eyes
	virtual const QAngle&	LocalEyeAngles( void ) = 0;	// Direction of eyes in local space (pl.v_angle)
